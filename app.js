...
mongoose.connect('mongodb+srv://admin-josiah:cuBzxcuk@cluster0.0w8pg5q.mongodb.net/journal')
...
app.get('/posts/:postName', (req, res) => {
  const requestParamValue = req.params.postName;
  Post.find({}, function (err, foundPosts) {
    foundPosts.forEach((post) => {
      if (_.lowerCase(post.title) === _.lowerCase(requestParamValue)) {
...
app.post('/compose', (req, res) => {
  ...
  post.save(function (err, post) {
    if (!err) {
      console.log(`Successfully saved post with title ${post.title} to database.`);
    }
  });
  res.redirect("/");
});
